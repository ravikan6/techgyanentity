import { CreatorEditBrand, CreatorInfoEdit, CreatorEditSections } from '@/components/studio/author';
import { DecryptAuthorStudioCookie } from '@/lib/actions/studio';
import { redirect } from 'next/navigation';
import React from 'react'

export function metadata({ searchParams }) {
    let page = searchParams?.t || '';

    if (page === 'sections') {
        return {
            title: 'Sections',
            description: 'Edit your channel sections',
            keywords: 'sections, channel, edit'
        };
    } else if (page === 'branding') {
        return {
            title: 'Branding',
            description: 'Edit your channel branding',
            keywords: 'branding, channel, edit'
        };
    } else if (page === 'info') {
        return {
            title: 'Basic Details',
            description: 'Edit your channel info',
            keywords: 'info, channel, edit'
        };
    } else {
        return {
            title: 'Sections',
            description: 'Edit your channel sections',
            keywords: 'sections, channel, edit'
        };
    };
};

const ChannelEditPage = async ({ searchParams }) => {
    let page = searchParams?.t || '';
    let data = {}
    let authorInfo = await DecryptAuthorStudioCookie();
    if (!authorInfo) {
        redirect(`/${process.env.STUDIO_URL_PREFIX}/dashboard`);
    }
    if (page === 'sections') {
    } else if (page === 'branding') {
        data = { ...authorInfo };
        data.media = { image: authorInfo?.image, banner: authorInfo?.banner };
        data.logo = authorInfo?.image?.url;
        data.banner = authorInfo?.banner?.url;
    } else if (page === 'info') {
        data = authorInfo;
    }

    if (page === 'sections') {
        return (
            <channel-edit-page>
                <CreatorEditSections data={data} />
            </channel-edit-page>
        )
    } else if (page === 'branding') {
        return (
            <channel-edit-page>
                <CreatorEditBrand data={data} />
            </channel-edit-page>
        )
    } else if (page === 'info') {
        return (
            <channel-edit-page>
                <CreatorInfoEdit data={data} />
            </channel-edit-page>
        )
    }
    redirect(`/${process.env.STUDIO_URL_PREFIX}/edit?t=sections`);
}

export default ChannelEditPage;